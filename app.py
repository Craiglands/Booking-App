...
EMAIL_PASSWORD = "xodxusryodrsjihf"   # App password (no spaces)
...
def init_database():
    try:
        ...
        if 'guests' not in columns:
            c.execute("ALTER TABLE bookings ADD COLUMN guests INTEGER DEFAULT 1")
        if 'room' not in columns:
            c.execute("ALTER TABLE bookings ADD COLUMN room TEXT")
        if 'surname' not in columns:
            c.execute("ALTER TABLE bookings ADD COLUMN surname TEXT")
        if 'filling' not in columns:
            c.execute("ALTER TABLE bookings ADD COLUMN filling TEXT")
        if 'bread' not in columns:
            c.execute("ALTER TABLE bookings ADD COLUMN bread TEXT")
        if 'collection_time' not in columns:
            c.execute("ALTER TABLE bookings ADD COLUMN collection_time TEXT")
        if 'guest_email' not in columns:
            c.execute("ALTER TABLE bookings ADD COLUMN guest_email TEXT")
        if 'confirmation_sent' not in columns:
            c.execute("ALTER TABLE bookings ADD COLUMN confirmation_sent INTEGER DEFAULT 0")
        if 'is_deleted' not in columns:
            c.execute("ALTER TABLE bookings ADD COLUMN is_deleted INTEGER DEFAULT 0")
        
        c.execute('''CREATE TABLE IF NOT EXISTS guest_meals (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def log_activity(action_type, booking_id=None, details=None):
    try:
        conn = sqlite3.connect('bookings.db')
        c = conn.cursor()
        c.execute('''INSERT INTO activity_log (action_type, booking_id, details)
                     VALUES (?, ?, ?)''', (action_type, booking_id, json.dumps(details) if details else None))
        conn.commit()
        conn.close()
    except Exception as e:
...
def send_customer_confirmation(booking_id, guest_email, is_cancellation=False):
    try:
        ...
        if booking['service'] == 'Packed Lunch':
            c.execute('SELECT * FROM guest_meals WHERE booking_id=? ORDER BY guest_number', (booking_id,))
...
"""
        ...
        # Also send a copy to the hotel (without CC)
        msg = MIMEMultipart()
        msg['From'] = EMAIL_SENDER
...
def send_future_bookings_backup(schedule_time="20:00"):
    try:
        ...
        c.execute('''SELECT * FROM bookings 
                     ...
                     ORDER BY date, service, time''', (tomorrow,))
        rows = c.fetchall()
...
def get_booking_count(date_str, service, time_slot):
    try:
        conn = get_db()
        c = conn.cursor()
        c.execute('SELECT COUNT(*) as count FROM bookings WHERE date=? AND service=? AND time=? AND is_deleted=0', 
...
def get_bookings_with_meals(bookings_rows):
    ...
    for booking in bookings:
        if booking['service'] == 'Packed Lunch':
            c = conn.cursor()
            c.execute('SELECT * FROM guest_meals WHERE booking_id=? ORDER BY guest_number', (booking['id'],))
...
def search_bookings(query):
    try:
        ...
        search_term = f"%{query}%"
        c.execute('''SELECT * FROM bookings 
                     WHERE (name LIKE ? OR room LIKE ? OR tel LIKE ? OR surname LIKE ? OR CAST(id AS TEXT) LIKE ?) AND is_deleted=0
...
def search_suggestions(query):
    try:
        ...
        c = conn.cursor()
        search_term = f"%{query}%"
        c.execute('''SELECT id, date, service, time, name, room, is_deleted 
                     FROM bookings 
                     WHERE name LIKE ? OR room LIKE ? OR tel LIKE ? OR surname LIKE ? OR CAST(id AS TEXT) LIKE ?
                     ORDER BY date DESC
                     LIMIT 20''', 
                  (search_term, search_term, search_term, search_term, search_term))
        rows = c.fetchall()
...
def get_logs_for_booking(booking_id):
    try:
        conn = get_db()
        c = conn.cursor()
        c.execute('''SELECT timestamp, action_type, details FROM activity_log 
                     WHERE booking_id = ? ORDER BY timestamp DESC''', (booking_id,))
        rows = c.fetchall()
        conn.close()
        return rows
    except Exception as e:
...
def index():
    ...
    try:
        if search_query:
            ...
            c = conn.cursor()
            if show_cancelled:
                c.execute('''SELECT * FROM bookings WHERE date=? ORDER BY service, time''', (selected_date,))
            else:
                c.execute('''SELECT * FROM bookings WHERE date=? AND is_deleted=0 ORDER BY service, time''', (selected_date,))
...
def get_booking_api(booking_id):
    try:
        ...
        if booking['is_deleted'] == 1:
            c.execute('''SELECT details FROM activity_log 
                         WHERE booking_id = ? AND action_type = 'delete' 
                         ORDER BY timestamp DESC LIMIT 1''', (booking_id,))
    ...
    try:
        ...
        if service == 'Packed Lunch':
            ...
            for i, guest in enumerate(data.get('guests', []), 1):
                c.execute('''INSERT INTO guest_meals 
    ...
    try:
        ...
        if data.get('service') == 'Packed Lunch':
            c.execute('''UPDATE bookings SET 
                         date=?, service=?, time=?, name=?, tel=?, guest_email=?,
                         notes=?, guests=?, room=?, surname=?, collection_time=?, paid=?
                         WHERE id=?''',
                      (data['date'], 'Packed Lunch', data['collection_time'],
                       data.get('name', ''), data.get('tel', ''),
                       data.get('guest_email', ''), data.get('notes', ''),
            ...
            c.execute('DELETE FROM guest_meals WHERE booking_id=?', (booking_id,))
            for i, guest in enumerate(data.get('guests', []), 1):
                c.execute('''INSERT INTO guest_meals 
                            (booking_id, guest_number, filling, bread, dietary)
                            VALUES (?, ?, ?, ?, ?)''',
            ...
            c.execute('''UPDATE bookings SET 
                         date=?, service=?, time=?, name=?, tel=?, guest_email=?,
                         voucher=?, notes=?, dietary=?, paid=?, room=?, surname=?, guests=?
                         WHERE id=?''',
    ...
    try:
        ...
        if success:
            if not is_cancellation:
                conn = get_db()
                c = conn.cursor()
                c.execute('UPDATE bookings SET confirmation_sent=1 WHERE id=?', (booking_id,))
                conn.commit()
                conn.close()
...
def export_bookings():
    ...
    try:
        ...
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute('''SELECT date, service, time, name, tel, guest_email, voucher, dietary, notes, paid, guests, room, surname
                     FROM bookings WHERE date=? AND is_deleted=0 ORDER BY service, time''', (selected_date,))
        rows = c.fetchall()
        conn.close()
            ...
            for _, row in df.iterrows():
                ...
                if mode == 'future_unique':
                    c.execute('''SELECT id FROM bookings 
                                 WHERE date=? AND service=? AND time=? AND name=? AND is_deleted=0''',
                              (data['date'], data['service'], data['time'], data['name']))
                    if c.fetchone():
                        skipped += 1
                        continue
    ...
    try:
        ...
        if view == 'upcoming':
            if show_cancelled:
                c.execute('''SELECT * FROM bookings 
                             WHERE date >= ? 
                             ORDER BY date, service, time''', (today,))
            else:
                c.execute('''SELECT * FROM bookings 
                             WHERE date >= ? AND is_deleted=0
                             ORDER BY date, service, time''', (today,))
            ...
            if show_cancelled:
                c.execute('''SELECT * FROM bookings 
                             ...
                             ORDER BY date DESC, service, time''', (start_date, end_date))
            else:
                c.execute('''SELECT * FROM bookings 
...
def export_range():
    ...
    try:
        ...
        if view == 'upcoming' or end_date == 'Future':
            today = get_today_date()
            query = '''SELECT * FROM bookings WHERE date >= ? AND is_deleted=0 ORDER BY date, service, time'''
            params = (today,)
            filename = f'craiglands_upcoming_{today}.xlsx'
        else:
            query = '''SELECT * FROM bookings WHERE date BETWEEN ? AND ? AND is_deleted=0 ORDER BY date DESC, service, time'''
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 70)
    print("DATA LOCATION: bookings.db")
